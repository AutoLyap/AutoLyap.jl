export ProximalPoint # set_gamma! is already exported from other structs

"""
    ProximalPoint(gamma)

A mutable struct representing the Proximal Point algorithm for a functional problem.
It subtypes the abstract `Algorithm` type.

# Fields
- `gamma::Float64`: The step-size parameter.
"""
mutable struct ProximalPoint <: Algorithm
    ...
    function ProximalPoint(gamma::Real)
...
ProximalPoint(; gamma::Real) = ProximalPoint(gamma)

# Define a specific setter method for this type for clarity and type-safety
function set_gamma!(alg::ProximalPoint, gamma::Real)
    alg.gamma = Float64(gamma)
    return alg
end

"""
    get_ABCD(alg::ProximalPoint, k::Int)

Return the state-space matrices for the Proximal Point algorithm.
The iteration `k` is unused as this is a time-invariant algorithm.
"""
function get_ABCD(alg::ProximalPoint, k::Int)
    γ = alg.gamma
